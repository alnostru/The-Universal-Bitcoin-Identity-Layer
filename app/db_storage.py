...
def create_user(pubkey: str, meta_data: Dict = None) -> str:
    """
    Create or get user by Bitcoin pubkey.

    Args:
        pubkey: Bitcoin public key
        meta_data: Additional user meta_data
...
def store_oauth_token(token_id: str, token_data: Dict) -> None:
    ...
    with session_scope() as session:
        token = OAuthToken(
            ...
            meta_data=token_data.get('meta_data')
        )
        session.add(token)
...
def get_session(session_id: str) -> Optional[Dict]:
    ...
    with session_scope() as session:
        ...
        return {
            ...
            'meta_data': user_session.meta_data
        }
...
def store_lnurl_challenge(session_id: str, challenge_data: Dict) -> None:
    ...
    with session_scope() as session:
        challenge = LNURLChallenge(
            ...
            meta_data=challenge_data.get('meta_data')
        )
